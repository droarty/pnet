  ...
  from (
   select gradeLevel, year, score, thisCnt, sum(cnt) totalToHere from (
    select a.score, a.gradeLevel, a.year, a.cnt thisCnt, b.score lessepsReads, b.cnt from (
     SELECT rIsat score, gradeLevel, year, count(*) cnt FROM dotalsne_pnet2.isbewodetails i
      where rIsat>0 and rIsat is not null and  test='PSAE' and (9=9)
      group by rIsat,year,gradeLevel) a
     left join (
     SELECT rIsat score, gradeLevel, year, count(*) cnt FROM dotalsne_pnet2.isbewodetails i2
      where rIsat>0 and rIsat is not null  and  test='PSAE' and (9=9)
   ...
   left join isbewo_tests s on (s.year=if(d.year<2006,2001,2006) and s.gradeLevel=d.gradeLevel and s.label='PSAE Reading')
   left join (select year, gradeLevel, label, below, meet, exceed from isbewo_tests) s13 on (if(d.year between 2006 and 2012,s13.year=2113,s13.year=d.year+100) and s13.gradeLevel=d.gradeLevel and s13.label='PSAE Reading')
  ...
  from (
   select gradeLevel, year, score, thisCnt, sum(cnt) totalToHere from (
    select a.score, a.gradeLevel, a.year, a.cnt thisCnt, b.score lessepsMaths, b.cnt from (
     SELECT mIsat score, gradeLevel, year, count(*) cnt FROM dotalsne_pnet2.isbewodetails i
      where mIsat>0 and mIsat is not null and  test='PSAE' and (9=9)
      group by mIsat,year,gradeLevel) a
     left join (
     SELECT mIsat score, gradeLevel, year, count(*) cnt FROM dotalsne_pnet2.isbewodetails i2
      where mIsat>0 and mIsat is not null and  test='PSAE' and (9=9)
       group by mIsat,year,gradeLevel) b
     on (a.score>=b.score and a.year=b.year and a.gradeLevel=b.gradeLevel)
    ) c
    group by score, thisCnt, gradeLevel, year) d
   left join isbewo_tests s on (s.year=if(d.year<2006,2001,2006) and s.gradeLevel=d.gradeLevel and s.label='PSAE Mathematics')
   left join (select year, gradeLevel, label, below, meet, exceed from isbewo_tests) s13 on (if(d.year between 2006 and 2012,s13.year=2113,s13.year=d.year+100) and s13.gradeLevel=d.gradeLevel and s13.label='PSAE Mathematics')
group by d.year, d.gradeLevel
) g
union
select g.*,'8=8' q, 'PSAE' test,
  ...
  from (
   select gradeLevel, year, score, thisCnt, sum(cnt) totalToHere from (
    select a.score, a.gradeLevel, a.year, a.cnt thisCnt, b.score lessepsScis, b.cnt from (
     SELECT sciIsat score, gradeLevel, year, count(*) cnt FROM dotalsne_pnet2.isbewodetails i
      where sciIsat>0 and sciIsat is not null and  test='PSAE' and (9=9)
      group by sciIsat,year,gradeLevel) a
     left join (
     SELECT sciIsat score, gradeLevel, year, count(*) cnt FROM dotalsne_pnet2.isbewodetails i2
      where sciIsat>0 and sciIsat is not null and  test='PSAE' and (9=9)
       group by sciIsat,year,gradeLevel) b
     on (a.score>=b.score and a.year=b.year and a.gradeLevel=b.gradeLevel)
    ) c
    group by score, thisCnt, gradeLevel, year) d
   left join isbewo_tests s on (s.year=if(d.year<2006,2001,2006) and s.gradeLevel=d.gradeLevel and s.label='PSAE Science')
   left join (select  year,gradeLevel, label, below, meet, exceed from isbewo_tests) s13 on (if(d.year between 2006 and 2012,s13.year=2113,s13.year=d.year+100) and s13.gradeLevel=d.gradeLevel and s13.label='PSAE Science')
group by d.year, d.gradeLevel
) g
union
...
select d.year, d.gradeLevel, d.year-d.gradeLevel+12 cohort,
  ...
  from (
   select gradeLevel, year, score, thisCnt, sum(cnt) totalToHere from (
    ...
    group by score, thisCnt, gradeLevel, year) d
   left join isbewo_tests s on (s.year=if(d.year<2006,2001,2006) and s.gradeLevel=d.gradeLevel and s.label='ISAT Science')
   left join (select  year,gradeLevel, label, below, meet, exceed from isbewo_tests) s13 on (if(d.year between 2006 and 2012,s13.year=2113,s13.year=d.year+100) and s13.gradeLevel=d.gradeLevel and s13.label='ISAT Science')
group by d.year, d.gradeLevel
) g
union
select g.*,'8=8' q, 'ISAT' test,
...
select d.year, d.gradeLevel, d.year-d.gradeLevel+12 cohort,
  ...
  from (
   select gradeLevel, year, score, thisCnt, sum(cnt) totalToHere from (
    ...
    group by score, thisCnt, gradeLevel, year) d
   left join isbewo_tests s on (s.year=if(d.year<2006,2001,2006) and s.gradeLevel=d.gradeLevel and s.label='ISAT Mathematics')
   left join (select year, gradeLevel, label, below, meet, exceed from isbewo_tests) s13 on (if(d.year between 2006 and 2012,s13.year=2113,s13.year=d.year+100) and s13.gradeLevel=d.gradeLevel and s13.label='ISAT Mathematics')
group by d.year, d.gradeLevel
) g
union
select g.*,'8=8' q, 'ISAT' test,
...
select d.year, d.gradeLevel,d.year-d.gradeLevel+12 cohort,
  ...
  from (
   select gradeLevel, year, score, thisCnt, sum(cnt) totalToHere from (
    ...
    group by score, thisCnt, gradeLevel, year) d
   left join isbewo_tests s on (s.year=if(d.year<2006,2001,2006) and s.gradeLevel=d.gradeLevel and s.label='ISAT Reading')
   left join (select year, gradeLevel, label, below, meet, exceed from isbewo_tests) s13 on (if(d.year between 2006 and 2012,s13.year=2113,s13.year=d.year+100) and s13.gradeLevel=d.gradeLevel and s13.label='ISAT Reading')
group by d.year, d.gradeLevel
) g
) h 
