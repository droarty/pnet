...
if($schId){
  db_set_active('isbewo');
	...
	$q=<<<UINQ
insert into isbewo_tests (label,subject,tablename,fieldName, year,gradeLevel,totalCnt,
...
/*    #c) get list of scores joined with scores of equal or lower value for each grade/year...*/
    select a.score, a.gradeLevel, a.year, a.cnt thisCnt, b.score lesserIsats, b.cnt from (
     SELECT rIsat score, gradeLevel, year, count(*) cnt FROM isbewodetails i where rIsat>0  and cdt='x' and test='ISAT'     group by rIsat,year,gradeLevel) a
     left join (
     SELECT rIsat score, gradeLevel, year, count(*) cnt FROM isbewodetails i2 where rIsat>0 and cdt='x' and test='ISAT'
   ...
   inner join (
    select year eyr, gradeLevel egrd, count(*) totCnt, min(rIsat) minScore, max(rIsat) maxScore FROM
     isbewodetails where rIsat>0 and cdt='x' and test='ISAT'  group by year, gradeLevel) e
...
left join isbewo_tests i on (i.year=h.year and i.gradeLevel=h.gradeLevel and i.label=h.label)
where i.id is null
UINQ;
  db_query($q);
...
/*    #c) get list of scores joined with scores of equal or lower value for each grade/year...*/
    select a.score, a.gradeLevel, a.year, a.cnt thisCnt, b.score lessemIsats, b.cnt from (
     SELECT mIsat score, gradeLevel, year, count(*) cnt FROM isbewodetails i where mIsat>0  and cdt='x' and test='ISAT'     group by mIsat,year,gradeLevel) a
     left join (
     SELECT mIsat score, gradeLevel, year, count(*) cnt FROM isbewodetails i2 where mIsat>0 and cdt='x' and test='ISAT'
       group by mIsat,year,gradeLevel) b
     on (a.score>=b.score and a.year=b.year and a.gradeLevel=b.gradeLevel) ) c
    group by score, thisCnt, gradeLevel, year) d
   inner join (
    select year eyr, gradeLevel egrd, count(*) totCnt, min(mIsat) minScore, max(mIsat) maxScore FROM
     isbewodetails where mIsat>0 and cdt='x' and test='ISAT'  group by year, gradeLevel) e
   on (d.year=e.eyr and d.gradeLevel=e.egrd)
   left join stanine s on (s.topPct*e.totCnt/100) between d.totalToHere-thisCnt and totalToHere) f
   where stanine is not null) g
 group by year, gradeLevel) h
...
/*    #c) get list of scores joined with scores of equal or lower value for each grade/year...*/
    select a.score, a.gradeLevel, a.year, a.cnt thisCnt, b.score lessesciIsats, b.cnt from (
     SELECT sciIsat score, gradeLevel, year, count(*) cnt FROM isbewodetails i where sciIsat>0  and cdt='x' and test='ISAT'     group by sciIsat,year,gradeLevel) a
     left join (
     SELECT sciIsat score, gradeLevel, year, count(*) cnt FROM isbewodetails i2 where sciIsat>0 and cdt='x' and test='ISAT'
   ...
   inner join (
    select year eyr, gradeLevel egrd, count(*) totCnt, min(sciIsat) minScore, max(sciIsat) maxScore FROM
     isbewodetails where sciIsat>0 and cdt='x' and test='ISAT'  group by year, gradeLevel) e
   on (d.year=e.eyr and d.gradeLevel=e.egrd)
   left join stanine s on (s.topPct*e.totCnt/100) between d.totalToHere-thisCnt and totalToHere) f
   where stanine is not null) g
 group by year, gradeLevel) h
...
/*    #c) get list of scores joined with scores of equal or lower value for each grade/year...*/
    select a.score, a.gradeLevel, a.year, a.cnt thisCnt, b.score lessesciIsats, b.cnt from (
     SELECT sciIsat score, gradeLevel, year, count(*) cnt FROM isbewodetails i where sciIsat>0  and cdt='x' and test='PSAE'     group by sciIsat,year,gradeLevel) a
     left join (
     SELECT sciIsat score, gradeLevel, year, count(*) cnt FROM isbewodetails i2 where sciIsat>0 and cdt='x' and test='PSAE'
       group by sciIsat,year,gradeLevel) b
     on (a.score>=b.score and a.year=b.year and a.gradeLevel=b.gradeLevel) ) c
    group by score, thisCnt, gradeLevel, year) d
   inner join (
    select year eyr, gradeLevel egrd, count(*) totCnt, min(sciIsat) minScore, max(sciIsat) maxScore FROM
     isbewodetails where sciIsat>0 and cdt='x' and test='PSAE'  group by year, gradeLevel) e
   on (d.year=e.eyr and d.gradeLevel=e.egrd)
   left join stanine s on (s.topPct*e.totCnt/100) between d.totalToHere-thisCnt and totalToHere) f
   where stanine is not null) g
 group by year, gradeLevel) h
...
/*    #c) get list of scores joined with scores of equal or lower value for each grade/year...*/
    select a.score, a.gradeLevel, a.year, a.cnt thisCnt, b.score lesserIsats, b.cnt from (
     SELECT rIsat score, gradeLevel, year, count(*) cnt FROM isbewodetails i where rIsat>0  and cdt='x' and test='PSAE'     group by rIsat,year,gradeLevel) a
     left join (
     SELECT rIsat score, gradeLevel, year, count(*) cnt FROM isbewodetails i2 where rIsat>0 and cdt='x' and test='PSAE'
       group by rIsat,year,gradeLevel) b
     on (a.score>=b.score and a.year=b.year and a.gradeLevel=b.gradeLevel) ) c
    group by score, thisCnt, gradeLevel, year) d
   inner join (
    select year eyr, gradeLevel egrd, count(*) totCnt, min(rIsat) minScore, max(rIsat) maxScore FROM
     isbewodetails where rIsat>0 and cdt='x' and test='PSAE'  group by year, gradeLevel) e
   on (d.year=e.eyr and d.gradeLevel=e.egrd)
   left join stanine s on (s.topPct*e.totCnt/100) between d.totalToHere-thisCnt and totalToHere) f
   where stanine is not null) g
 group by year, gradeLevel) h
...
/*    #c) get list of scores joined with scores of equal or lower value for each grade/year...*/
    select a.score, a.gradeLevel, a.year, a.cnt thisCnt, b.score lessemIsats, b.cnt from (
     SELECT mIsat score, gradeLevel, year, count(*) cnt FROM isbewodetails i where mIsat>0  and cdt='x' and test='PSAE'     group by mIsat,year,gradeLevel) a
     left join (
     SELECT mIsat score, gradeLevel, year, count(*) cnt FROM isbewodetails i2 where mIsat>0 and cdt='x' and test='PSAE'
       group by mIsat,year,gradeLevel) b
     on (a.score>=b.score and a.year=b.year and a.gradeLevel=b.gradeLevel) ) c
    group by score, thisCnt, gradeLevel, year) d
   inner join (
    select year eyr, gradeLevel egrd, count(*) totCnt, min(mIsat) minScore, max(mIsat) maxScore FROM
     isbewodetails where mIsat>0 and cdt='x' and test='PSAE'  group by year, gradeLevel) e
   on (d.year=e.eyr and d.gradeLevel=e.egrd)
   left join stanine s on (s.topPct*e.totCnt/100) between d.totalToHere-thisCnt and totalToHere) f
   where stanine is not null) g
 group by year, gradeLevel) h
...
SELECT cdts,  d.year, d.gradeLevel, d.year-d.gradeLevel+12 cohort,
  ...
  from (
   ...
   left join isbewo_tests s on (s.year=d.year and s.gradeLevel=d.gradeLevel and s.label='PSAE Reading')
...
SELECT cdts,  d.year, d.gradeLevel, d.year-d.gradeLevel+12 cohort,
  ...
  from (
   ...
   left join isbewo_tests s on (s.year=d.year and s.gradeLevel=d.gradeLevel and s.label='PSAE Mathematics')
group by cdts,  d.year, d.gradeLevel
) g
union
SELECT g.*,
...
SELECT cdts,  d.year, d.gradeLevel, d.year-d.gradeLevel+12 cohort,
  ...
  from (
   SELECT cdts,  gradeLevel, year, score, thisCnt, sum(cnt) totalToHere from (
    ...
    group by cdts,  score, thisCnt, gradeLevel, year) d
   left join isbewo_tests s on (s.year=d.year and s.gradeLevel=d.gradeLevel and s.label='PSAE Science')
group by cdts,  d.year, d.gradeLevel
) g
union
SELECT g.*,
...
SELECT cdts,  d.year, d.gradeLevel, d.year-d.gradeLevel+12 cohort,
  ...
  from (
   SELECT cdts,  gradeLevel, year, score, thisCnt, sum(cnt) totalToHere from (
    ...
    group by cdts,  score, thisCnt, gradeLevel, year) d
   left join isbewo_tests s on (s.year=d.year and s.gradeLevel=d.gradeLevel and s.label='ISAT Science')
group by cdts,  d.year, d.gradeLevel
) g
union
SELECT g.*,
...
SELECT cdts,  d.year, d.gradeLevel, d.year-d.gradeLevel+12 cohort,
  ...
  from (
   SELECT cdts,  gradeLevel, year, score, thisCnt, sum(cnt) totalToHere from (
    ...
    group by cdts,  score, thisCnt, gradeLevel, year) d
   left join isbewo_tests s on (s.year=d.year and s.gradeLevel=d.gradeLevel and s.label='ISAT Mathematics')
group by cdts,  d.year, d.gradeLevel
) g
union
SELECT g.*,
...
SELECT cdts,  d.year, d.gradeLevel,d.year-d.gradeLevel+12 cohort,
  ...
  from (
   SELECT cdts,  gradeLevel, year, score, thisCnt, sum(cnt) totalToHere from (
    ...
    group by cdts,  score, thisCnt, gradeLevel, year) d
   left join isbewo_tests s on (s.year=d.year and s.gradeLevel=d.gradeLevel and s.label='ISAT Reading')
group by cdts,  d.year, d.gradeLevel
) g
) h1
...
<body>
    <a href="woupdatecomposites.php?baselines=t">Update Baselines</a> Takes only the statewide data, ignores CPS data.  This may take several minutes... but if it does not return, it is safe to rerun it.  This script will create the normalized stanine boundaries based on the data from the  entire state. It only does this for new data so if you added a year of raw data, it will create a baseline record in the isbewo_tests table for each grade level and each test for that year. <b>Don't forget to add cutoff scores to isbewo_tests after this step and before running composites...</b><br><br>
<a href="woupdatecomposites.php?composites=t">Update Composites</a>  Run this after running baselines... This may take several minutes... but if it does not return, it is safe to rerun it.  This script will calculate all the composites for each year for each school and then do the same for math and reading.  This is stored in the woschwideComposites table.<br><br>
<a href="woupdatecomposites.php?growth=t">Update Growth Table</a>  Run this after updating composites.. this will generate the table of growth for the most recent year.  It will create a growth in composite, math, reading and going back one year, two, three, four, and five years.<br><br>
<a href="woupdatecomposites.php?exceptions=t">Update Exceptions</a>  Run this after running baselines and composites... This may take several minutes... but if it does not return, it is safe to rerun it.  This script will determine if any combinations of filters create a sub group smaller than 10... it is not currently used for anything but could be.<br><br>
<a href="woupdatecomposites.php?clearfilters=t">Clear all Filters</a>  Run this after running baselines and composites and Exceptions.. Every time you query from the public site, the new query will be run (and it may take a while) and will be stored in the filters table.  It is safe to rerun this any time.  This script dumps all old filters that are cached.<br><br>......
<script>
