...
if($schId){
  db_set_active('isbewo');

	$q=<<<UINQ
insert into isbexchiwo_tests (label,subject,tablename,fieldName, year,gradeLevel,totalCnt,
...
/*    #c) get list of scores joined with scores of equal or lower value for each grade/year...*/
    select a.score, a.gradeLevel, a.year, a.cnt thisCnt, b.score lesserIsats, b.cnt from (
     SELECT rIsat score, gradeLevel, year, count(*) cnt FROM isbewodetails i where rIsat>0  and cdt='x' and ismatch is null and  test='ISAT'     group by rIsat,year,gradeLevel) a
     left join (
     SELECT rIsat score, gradeLevel, year, count(*) cnt FROM isbewodetails i2 where rIsat>0 and cdt='x' and ismatch is null and  test='ISAT'
   ...
   inner join (
    select year eyr, gradeLevel egrd, count(*) totCnt, min(rIsat) minScore, max(rIsat) maxScore FROM
     isbewodetails where rIsat>0 and cdt='x' and ismatch is null and  test='ISAT'  group by year, gradeLevel) e
...
left join isbexchiwo_tests i on (i.year=h.year and i.gradeLevel=h.gradeLevel and i.label=h.label)
where i.id is null
UINQ;
  db_query($q);
...
insert into isbexchiwo_tests (label,subject,tablename,fieldName, year,gradeLevel,totalCnt,
  ...
  s67pct,s78,s78pct,s89,s89pct,q12,q12pct,q23,q23pct,q34,q34pct)
select h.* from (
select Convert('ISAT Mathematics' USING utf8) label, 'Math' subject, 'isbewodetails' tableName,
...
/*    #c) get list of scores joined with scores of equal or lower value for each grade/year...*/
    select a.score, a.gradeLevel, a.year, a.cnt thisCnt, b.score lessemIsats, b.cnt from (
     SELECT mIsat score, gradeLevel, year, count(*) cnt FROM isbewodetails i where mIsat>0  and cdt='x' and ismatch is null and  test='ISAT'     group by mIsat,year,gradeLevel) a
     left join (
     SELECT mIsat score, gradeLevel, year, count(*) cnt FROM isbewodetails i2 where mIsat>0 and cdt='x' and ismatch is null and  test='ISAT'
       group by mIsat,year,gradeLevel) b
     on (a.score>=b.score and a.year=b.year and a.gradeLevel=b.gradeLevel) ) c
    group by score, thisCnt, gradeLevel, year) d
   inner join (
    select year eyr, gradeLevel egrd, count(*) totCnt, min(mIsat) minScore, max(mIsat) maxScore FROM
     isbewodetails where mIsat>0 and cdt='x' and ismatch is null and  test='ISAT'  group by year, gradeLevel) e
   on (d.year=e.eyr and d.gradeLevel=e.egrd)
   left join stanine s on (s.topPct*e.totCnt/100) between d.totalToHere-thisCnt and totalToHere) f
   where stanine is not null) g
 group by year, gradeLevel) h
...
/*    #c) get list of scores joined with scores of equal or lower value for each grade/year...*/
    select a.score, a.gradeLevel, a.year, a.cnt thisCnt, b.score lessesciIsats, b.cnt from (
     SELECT sciIsat score, gradeLevel, year, count(*) cnt FROM isbewodetails i where sciIsat>0  and cdt='x' and ismatch is null and  test='ISAT'     group by sciIsat,year,gradeLevel) a
     left join (
     SELECT sciIsat score, gradeLevel, year, count(*) cnt FROM isbewodetails i2 where sciIsat>0 and cdt='x' and ismatch is null and  test='ISAT'
   ...
   inner join (
    select year eyr, gradeLevel egrd, count(*) totCnt, min(sciIsat) minScore, max(sciIsat) maxScore FROM
     isbewodetails where sciIsat>0 and cdt='x' and ismatch is null and  test='ISAT'  group by year, gradeLevel) e
   on (d.year=e.eyr and d.gradeLevel=e.egrd)
   left join stanine s on (s.topPct*e.totCnt/100) between d.totalToHere-thisCnt and totalToHere) f
   where stanine is not null) g
 group by year, gradeLevel) h
...
/*    #c) get list of scores joined with scores of equal or lower value for each grade/year...*/
    select a.score, a.gradeLevel, a.year, a.cnt thisCnt, b.score lessesciIsats, b.cnt from (
     SELECT sciIsat score, gradeLevel, year, count(*) cnt FROM isbewodetails i where sciIsat>0  and cdt='x' and ismatch is null and  test='PSAE'     group by sciIsat,year,gradeLevel) a
     left join (
     SELECT sciIsat score, gradeLevel, year, count(*) cnt FROM isbewodetails i2 where sciIsat>0 and cdt='x' and ismatch is null and  test='PSAE'
       group by sciIsat,year,gradeLevel) b
     on (a.score>=b.score and a.year=b.year and a.gradeLevel=b.gradeLevel) ) c
    group by score, thisCnt, gradeLevel, year) d
   inner join (
    select year eyr, gradeLevel egrd, count(*) totCnt, min(sciIsat) minScore, max(sciIsat) maxScore FROM
     isbewodetails where sciIsat>0 and cdt='x' and ismatch is null and  test='PSAE'  group by year, gradeLevel) e
   on (d.year=e.eyr and d.gradeLevel=e.egrd)
   left join stanine s on (s.topPct*e.totCnt/100) between d.totalToHere-thisCnt and totalToHere) f
   where stanine is not null) g
 group by year, gradeLevel) h
...
/*    #c) get list of scores joined with scores of equal or lower value for each grade/year...*/
    select a.score, a.gradeLevel, a.year, a.cnt thisCnt, b.score lesserIsats, b.cnt from (
     SELECT rIsat score, gradeLevel, year, count(*) cnt FROM isbewodetails i where rIsat>0  and cdt='x' and ismatch is null and  test='PSAE'     group by rIsat,year,gradeLevel) a
     left join (
     SELECT rIsat score, gradeLevel, year, count(*) cnt FROM isbewodetails i2 where rIsat>0 and cdt='x' and ismatch is null and  test='PSAE'
       group by rIsat,year,gradeLevel) b
     on (a.score>=b.score and a.year=b.year and a.gradeLevel=b.gradeLevel) ) c
    group by score, thisCnt, gradeLevel, year) d
   inner join (
    select year eyr, gradeLevel egrd, count(*) totCnt, min(rIsat) minScore, max(rIsat) maxScore FROM
     isbewodetails where rIsat>0 and cdt='x' and ismatch is null and  test='PSAE'  group by year, gradeLevel) e
   on (d.year=e.eyr and d.gradeLevel=e.egrd)
   left join stanine s on (s.topPct*e.totCnt/100) between d.totalToHere-thisCnt and totalToHere) f
   where stanine is not null) g
 group by year, gradeLevel) h
...
/*    #c) get list of scores joined with scores of equal or lower value for each grade/year...*/
    select a.score, a.gradeLevel, a.year, a.cnt thisCnt, b.score lessemIsats, b.cnt from (
     SELECT mIsat score, gradeLevel, year, count(*) cnt FROM isbewodetails i where mIsat>0  and cdt='x' and ismatch is null and  test='PSAE'     group by mIsat,year,gradeLevel) a
     left join (
     SELECT mIsat score, gradeLevel, year, count(*) cnt FROM isbewodetails i2 where mIsat>0 and cdt='x' and ismatch is null and  test='PSAE'
       group by mIsat,year,gradeLevel) b
     on (a.score>=b.score and a.year=b.year and a.gradeLevel=b.gradeLevel) ) c
    group by score, thisCnt, gradeLevel, year) d
   inner join (
    select year eyr, gradeLevel egrd, count(*) totCnt, min(mIsat) minScore, max(mIsat) maxScore FROM
     isbewodetails where mIsat>0 and cdt='x' and ismatch is null and  test='PSAE'  group by year, gradeLevel) e
   on (d.year=e.eyr and d.gradeLevel=e.egrd)
   left join stanine s on (s.topPct*e.totCnt/100) between d.totalToHere-thisCnt and totalToHere) f
   where stanine is not null) g
 group by year, gradeLevel) h
...
//first do the Composites... then Math... then Reading...
    $qcomp=<<<UINQ
	insert into woschwideComposites
  ...
  from (
   SELECT cdts,  gradeLevel, year, score, thisCnt, sum(cnt) totalToHere from (
    SELECT a.cdts,  a.score, a.gradeLevel, a.year, a.cnt thisCnt, b.score lessepsReads, b.cnt from (
     SELECT cdts,  rIsat score, gradeLevel, year, count(*) cnt FROM isbewodetails i
      where rIsat>0 and rIsat is not null and  test='PSAE' and cdts ='%s' and year=%d 
      group by cdts,  rIsat,year,gradeLevel) a
     left join (
     SELECT cdts,  rIsat score, gradeLevel, year, count(*) cnt FROM isbewodetails i2
      where rIsat>0 and rIsat is not null  and  test='PSAE' and cdts ='%s' and year=%d 
   ...
   left join isbexchiwo_tests s on (s.year=d.year and s.gradeLevel=d.gradeLevel and s.label='PSAE Reading')
  ...
  from (
   SELECT cdts,  gradeLevel, year, score, thisCnt, sum(cnt) totalToHere from (
    SELECT a.cdts,  a.score, a.gradeLevel, a.year, a.cnt thisCnt, b.score lessepsMaths, b.cnt from (
     SELECT cdts,  mIsat score, gradeLevel, year, count(*) cnt FROM isbewodetails i
      where mIsat>0 and mIsat is not null and  test='PSAE' and cdts ='%s' and year=%d 
      group by cdts,  mIsat,year,gradeLevel) a
     left join (
     SELECT cdts,  mIsat score, gradeLevel, year, count(*) cnt FROM isbewodetails i2
      where mIsat>0 and mIsat is not null and  test='PSAE' and cdts ='%s' and year=%d 
   ...
   left join isbexchiwo_tests s on (s.year=d.year and s.gradeLevel=d.gradeLevel and s.label='PSAE Mathematics')
group by cdts,  d.year, d.gradeLevel
) g
union
SELECT g.*,
  ...
  from (
   SELECT cdts,  gradeLevel, year, score, thisCnt, sum(cnt) totalToHere from (
    SELECT a.cdts,  a.score, a.gradeLevel, a.year, a.cnt thisCnt, b.score lessepsScis, b.cnt from (
     SELECT cdts,  sciIsat score, gradeLevel, year, count(*) cnt FROM isbewodetails i
      where sciIsat>0 and sciIsat is not null and  test='PSAE' and cdts ='%s' and year=%d 
      group by cdts,  sciIsat,year,gradeLevel) a
     left join (
     SELECT cdts,  sciIsat score, gradeLevel, year, count(*) cnt FROM isbewodetails i2
      where sciIsat>0 and sciIsat is not null and  test='PSAE' and cdts ='%s' and year=%d 
       group by cdts,  sciIsat,year,gradeLevel) b
     on (a.score>=b.score and a.year=b.year and a.gradeLevel=b.gradeLevel and a.cdts=b.cdts)
    ) c
    group by cdts,  score, thisCnt, gradeLevel, year) d
   left join isbexchiwo_tests s on (s.year=d.year and s.gradeLevel=d.gradeLevel and s.label='PSAE Science')
group by cdts,  d.year, d.gradeLevel
) g
union
SELECT g.*,
...
SELECT cdts,  d.year, d.gradeLevel, d.year-d.gradeLevel+12 cohort,
  ...
  from (
   SELECT cdts,  gradeLevel, year, score, thisCnt, sum(cnt) totalToHere from (
    ...
    group by cdts,  score, thisCnt, gradeLevel, year) d
   left join isbexchiwo_tests s on (s.year=d.year and s.gradeLevel=d.gradeLevel and s.label='ISAT Science')
group by cdts,  d.year, d.gradeLevel
) g
union
SELECT g.*,
...
SELECT cdts,  d.year, d.gradeLevel, d.year-d.gradeLevel+12 cohort,
  ...
  from (
   SELECT cdts,  gradeLevel, year, score, thisCnt, sum(cnt) totalToHere from (
    ...
    group by cdts,  score, thisCnt, gradeLevel, year) d
   left join isbexchiwo_tests s on (s.year=d.year and s.gradeLevel=d.gradeLevel and s.label='ISAT Mathematics')
group by cdts,  d.year, d.gradeLevel
) g
union
SELECT g.*,
...
SELECT cdts,  d.year, d.gradeLevel,d.year-d.gradeLevel+12 cohort,
  ...
  from (
   SELECT cdts,  gradeLevel, year, score, thisCnt, sum(cnt) totalToHere from (
    ...
    group by cdts,  score, thisCnt, gradeLevel, year) d
   left join isbexchiwo_tests s on (s.year=d.year and s.gradeLevel=d.gradeLevel and s.label='ISAT Reading')
group by cdts,  d.year, d.gradeLevel
) g
) h1
...
left join tblsitemaster t on h.cdts=t.cdts
left join woschwideComposites swc on (h.cdts=swc.cdts and h.year=swc.year and subj='Composite')
...
(SELECT cdts, year, count(*) cnt FROM isbewodetails i  group by cdts, year) a
left join woschwideComposites s on (s.cdts=a.cdts and s.year=a.year and s.subj='Composite')
...
$i=0;
$cnt=db_result(db_query("SELECT count(*) cnt FROM woschwideComposites where subj='Composite'"));
...
$cnt2=db_result(db_query("SELECT count(*) cnt FROM woschwideComposites where subj='Composite'"));
...
left join tblsitemaster t on h.cdts=t.cdts
left join woschwideComposites swc on (h.cdts=swc.cdts and h.year=swc.year and subj='Math')
where swc.cdts is null and swc.year is null
group by t.shortlabel, h.cdts,  h.year
UINQ;
...
select a.cdts, a.year yr from
(SELECT cdts, year, count(*) cnt FROM isbewodetails i  group by cdts, year) a
left join woschwideComposites s on (s.cdts=a.cdts and s.year=a.year and s.subj='Math')
where s.year is null and cnt>10 and a.cdts is not null and a.cdts!=''
UINQ;
db_set_active('isbewo');
$r=db_query($q);
...
$i=0;
$cnt=db_result(db_query("SELECT count(*) cnt FROM woschwideComposites where subj='Math'"));
while($rw=db_fetch_array($r)){
...
$cnt2=db_result(db_query("SELECT count(*) cnt FROM woschwideComposites where subj='Math'"));
//if($i>298) $output.="<br> The maximum number of records was processed, keep running this command until all records have been created.";
...
left join tblsitemaster t on h.cdts=t.cdts
left join woschwideComposites swc on (h.cdts=swc.cdts and h.year=swc.year and subj='Reading')
where swc.cdts is null and swc.year is null
group by t.shortlabel, h.cdts,  h.year
UINQ;
...
select a.cdts, a.year yr from
(SELECT cdts, year, count(*) cnt FROM isbewodetails i  group by cdts, year) a
left join woschwideComposites s on (s.cdts=a.cdts and s.year=a.year and s.subj='Reading')
where s.year is null and cnt>10 and a.cdts is not null and a.cdts!=''
UINQ;
db_set_active('isbewo');
$r=db_query($q);
...
$i=0;
$cnt=db_result(db_query("SELECT count(*) cnt FROM woschwideComposites where subj='Reading'"));
while($rw=db_fetch_array($r)){
...
$cnt2=db_result(db_query("SELECT count(*) cnt FROM woschwideComposites where subj='Reading'"));
//if($i>298) $output.="<br> The maximum number of records was processed, keep running this command until all records have been created.";
...
if($grwRequest){
	$q=<<<UINQ
select max(year) from woschwideComposites
UINQ;
db_set_active('isbewo');
$yr=db_result(db_query($q));
$output.="<br>Done processing the woschwideGrowth table for the $yr year.  There were ".db_result(db_query("select count(*) from woschwideGrowth"))." records for all years before and now there are ";

    $q2=<<<UINQ
insert into woschwideGrowth (shortLabel, cdts, subj, year, yrsGrowth, stanineStart, stanineEnd, stanineGrowth, stanineGrowthPerYear, pmeGrowth, percentileGrowth,
...
FROM woschwideComposites s1
inner join woschwideComposites s2 on (s1.cdts=s2.cdts and s1.subj=s2.subj and s1.year=s2.year+1)
...
left join (select cdts ocdts, year oyear, yrsGrowth oyrsGrowth from woschwideGrowth) g on (c.cdts=ocdts and c.year=oyear and c.yrsGrowth=oyrsGrowth) where ocdts is null
UINQ;
if($yr){
    ...
    $output.=db_result(db_query("select count(*) from woschwideGrowth"))." records.<br>$q4<br>";
...
SELECT a.year, a.gradelevel, detcnt, i.cntcnt, totcnt, hasMinN, aaacnt
  FROM ( select gradelevel, year, count(*) detcnt FROM isbewodetails group by year, gradelevel) a
left join (select gradelevel, year, sum(if(fi='123',cnt,0)) cntcnt, sum(cnt) totcnt, sum(if(fi='123' and cnt!=minN,1,0)) hasMinN,sum(if(fi='',cnt,0)) aaacnt
  from isbewocnts group by year, gradelevel) i
...
//to create the '123' filter records...
	$q2d=<<<UINQ
delete from isbewocnts where year=2000 and gradelevel='05'
UINQ;
	$q2=<<<UINQ
insert into isbewocnts  (year, gradelevel, cdts, sex, eth, freelunch, cnt, schind, indind, minN, fi)
...
concat(ifnull(sex,''),'_',ifnull(eth,''),'_',ifnull(freelunch,'')) indind, count(*) minN, '123' fi
 FROM isbewodetails i where year=2000 and gradelevel='05' group by test, year, gradelevel,cdts,sex, eth, freelunch
UINQ;

//to set the minN values on '123' filters....
	$q3=<<<UINQ
update isbewocnts o inner join (
...
FROM (select id, year, gradelevel, cdts, schind,sex,freelunch, eth, cnt thiscnt from isbewocnts
  where fi='123'  and year=2000 and gradelevel='05') i
left join (select schind,sex,freelunch, eth, cnt sibcnts from isbewocnts
...
//to create the '1', '2', '3', '12', '13', '23' filters...
	$q4d=<<<UINQ
delete from isbewocnts where year=2000 and gradelevel='05' and fi!='123'
UINQ;
	$q4=<<<UINQ
insert into isbewocnts (year, gradelevel, cdts, schind, sex, eth, freelunch, cnt, minN, fi)
...
SELECT i.year, i.gradelevel, i.cdts, i.schind, i.sex,  'a' eth, 'a' freelunch,
  ...
  '1' fi
FROM (select year, gradelevel, cdts, schind,sex, sum(cnt) thiscnt from isbewocnts
  where fi='123' and year=2000 and gradelevel='05' group by schind,sex) i
left join (select schind,sex, sum(cnt) sibcnts from isbewocnts
...
SELECT i.year, i.gradelevel, i.cdts, i.schind, 'a' sex,  i.eth, 'a' freelunch,
  min(thiscnt) cnt, sum(sibcnts) minN1,
  '2' fi
FROM (select year, gradelevel, cdts, schind,eth, sum(cnt) thiscnt from isbewocnts
  where fi='123'  and year=2000 and gradelevel='05' group by schind,eth) i
left join (select schind,eth, sum(cnt) sibcnts from isbewocnts
...
SELECT i.year, i.gradelevel, i.cdts, i.schind, 'a' sex,  'a' eth, i.freelunch,
  min(thiscnt) cnt, sum(sibcnts) minN1,
  '3' fi
FROM (select year, gradelevel, cdts, schind,freelunch, sum(cnt) thiscnt from isbewocnts
  where fi='123'  and year=2000 and gradelevel='05' group by schind,freelunch) i
left join (select schind,freelunch, sum(cnt) sibcnts from isbewocnts
...
FROM (select year, gradelevel, cdts, schind,sex, eth, sum(cnt) thiscnt from isbewocnts
  where fi='123'  and year=2000 and gradelevel='05' group by schind,sex, eth) i
left join (select schind,sex, eth, sum(cnt) sibcnts from isbewocnts
...
FROM (select year, gradelevel, cdts, schind,sex, freelunch, sum(cnt) thiscnt from isbewocnts
  where fi='123'  and year=2000 and gradelevel='05' group by schind,sex, freelunch) i
left join (select schind,sex, freelunch, sum(cnt) sibcnts from isbewocnts
...
FROM (select year, gradelevel, cdts, schind,freelunch, eth, sum(cnt) thiscnt from isbewocnts
  where fi='123'  and year=2000 and gradelevel='05' group by schind,freelunch, eth) i
left join (select schind,freelunch, eth, sum(cnt) sibcnts from isbewocnts
...
//to create the 'a','a','a' filter or whole school filter....
	$q5d=<<<UINQ
delete from isbewocnts where year=2000 and gradelevel='05' and fi=''
UINQ;
	$q5=<<<UINQ
insert into isbewocnts (year, gradelevel, cdts, schind, sex, eth, freelunch, cnt, minN, fi)
select year, gradelevel, cdts, schind, sex, eth, freelunch, cnt,
...
UINQ;
	$q="select filter, count(*) cnt from isbewofilters group by filter";
	$output.=makeDS("noreport", $q, $colnames,'isbewo');
...
<body>
    <a href="xchiwoupdatecomposites.php?baselines=t">Update Baselines</a> Takes only the statewide data, ignores CPS data.  This may take several minutes... but if it does not return, it is safe to rerun it.  This script will create the normalized stanine boundaries based on the data from the  entire state. It only does this for new data so if you added a year of raw data, it will create a baseline record in the isbexchiwo_tests table for each grade level and each test for that year. <b>Don't forget to add cutoff scores to isbexchiwo_tests after this step and before running composites...</b><br><br>
<a href="xchiwoupdatecomposites.php?composites=t">Update Composites</a>  Run this after running baselines... This may take several minutes... but if it does not return, it is safe to rerun it.  This script will calculate all the composites for each year for each school and then do the same for math and reading.  This is stored in the woschwideComposites table.<br><br>
<a href="xchiwoupdatecomposites.php?growth=t">Update Growth Table</a>  Run this after updating composites.. this will generate the table of growth for the most recent year.  It will create a growth in composite, math, reading and going back one year, two, three, four, and five years.<br><br>
<a href="xchiwoupdatecomposites.php?exceptions=t">Update Exceptions</a>  Run this after running baselines and composites... This may take several minutes... but if it does not return, it is safe to rerun it.  This script will determine if any combinations of filters create a sub group smaller than 10... it is not currently used for anything but could be.<br><br>
<a href="xchiwoupdatecomposites.php?clearfilters=t">Clear all Filters</a>  Run this after running baselines and composites and Exceptions.. Every time you query from the public site, the new query will be run (and it may take a while) and will be stored in the filters table.  It is safe to rerun this any time.  This script dumps all old filters that are cached.<br><br>......
<script>
