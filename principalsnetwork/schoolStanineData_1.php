...
//DR 12/18/10 rearranged columns of raw data and added a few derived columns...

include_once $_SERVER[DOCUMENT_ROOT]."/includes/common.inc";
include_once $_SERVER[DOCUMENT_ROOT]."/includes/database.inc";
include_once $_SERVER[DOCUMENT_ROOT]."/includes/bootstrap.inc";
include_once $_SERVER[DOCUMENT_ROOT]."/includes/session.inc";
include_once $_SERVER[DOCUMENT_ROOT]."/includes/database.mysql.inc";
...
function addVar($varNm,$keyNode){
  $r=addJSVariableFromNode($varNm,$keyNode);
  return str_replace(array('</script>',"<script>","<script  type='text/javascript'>"),'',$r);  
};
...
//need to replace ' and make query safe...
	...
	$q="select cast(group_concat(r separator '\\n') as char) srch from (select cast(concat(schName,'(',city,')','|',schId) as char) r from iamfroa5_cte.tblsitemaster where ".$q." limit 40) a";
...
NAMEQ;
	  ...
	  from iamfroa5_cte.tblsitemaster
