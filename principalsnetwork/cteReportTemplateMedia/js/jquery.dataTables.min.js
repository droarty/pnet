...
var iPageCountHalf=Math.floor(iPageCount/2);var iPages=parseInt((oSettings.aiDisplay.length-1)/oSettings.iDisplayLength)+1;
var iCurrentPage=parseInt(oSettings.iDisplayStart/oSettings.iDisplayLength)+1;var sList="";
var iStartButton;var iEndButton;if(iPages<iPageCount){iStartButton=1;iEndButton=iPages
}else{if(iCurrentPage<=iPageCountHalf){iStartButton=1;iEndButton=iPageCount}else{if(iCurrentPage>=(iPages-iPageCountHalf)){iStartButton=iPages-iPageCount+1;
...
this.iCookieDuration=60*60*2}this.oApi={};this.fnDraw=function(){var oSettings=_fnSettingsFromNode(this[0]);
...
}}else{iCorrector++}}}}}return null};var bApi=false;for(var sFunc in $.fn.dataTableExt.oApi){this[sFunc]=function(){return $.fn.dataTableExt.oApi[sFunc].call(this,_fnSettingsFromNode(this[0]))
...
oSettings.aoData.push({_iId:oSettings.iNextId++,_aData:new Array(),nTr:this});oSettings.aiDisplayMaster.push(iThisIndex);
var aLocalData=oSettings.aoData[iThisIndex]._aData;$("td",this).each(function(i){aLocalData[i]=this.innerHTML
})});var iCorrector=0;for(var i=0;i<oSettings.aoColumns.length;i++){if(oSettings.aoColumns[i].sTitle==null){oSettings.aoColumns[i].sTitle=$("thead th:nth-child("+(i+1)+")",oSettings.nTable).html()
...
date.setTime(date.getTime()+(iSecs*1000));sName+="_"+window.location.pathname;document.cookie=sName+"="+sValue+"; expires="+date.toGMTString()+"; path=/"
}function _fnReadCookie(sName){var sNameEQ=sName+"_"+window.location.pathname+"=";
...
this.oApi._fnMasterIndexFromDisplay=_fnMasterIndexFromDisplay;this.oApi._fnDetectType=_fnDetectType;
this.oApi._fnGetDataMaster=_fnGetDataMaster;this.oApi._fnGetTrNodes=_fnGetTrNodes;
this.oApi._fnEscapeRegex=_fnEscapeRegex;this.oApi._fnSaveState=_fnSaveState;this.oApi._fnLoadState=_fnLoadState;
this.oApi._fnCreateCookie=_fnCreateCookie;this.oApi._fnReadCookie=_fnReadCookie}return this.each(function(){var oSettings=new classSettings();
