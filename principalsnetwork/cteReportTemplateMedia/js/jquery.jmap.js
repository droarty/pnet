...
Mapifies.Initialise = function ( element, options, callback ) {
	...
	//DR added to help out those like me who didn't understand the need for the Google include
	if(!GBrowserIsCompatible){
		jQuery(element).text('You are missing the Google Maps javascript include.  Visit Google Maps API pages to generate a key and learn how to include the API script.');
		return false;		
	};
	//DR end edit
	if (GBrowserIsCompatible()) {
		...
		return this.each(function(){
			if (method == 'init' && typeof options == 'undefined') {
				...
				try {
					...
					throw Error('Error in Mapifies Function ('+method+','+JSON.stringify(options)+'):'+err.message);//DR edited to provide more info...
				}
			}
		});
	}
