		...
		//	  window.dataSetObjects.showDevDisplay(tblnode, $("#"+tblnode));
			  if($("#"+tblnode).length>0&&(args.aaData&&args.aaData.length>0)){
				 ...
				 // var tbl2=$("#"+tblnode).find(".dataTables_info");
				  $("#"+node).prepend("<div id='"+node+"_copy'> (Copy Data)</div>");
				  var tblcopy=$("#"+node).find("#"+node+"_copy");
				  var makeClipButton=function(nd, txt){
					  window.setTimeout(function(){
					var clip = new ZeroClipboard.Client();
					clip.setText(txt);
					clip.setHandCursor(true);
					clip.addEventListener( 'onMouseUp', function(){
						alert("Data copied to clipboard, now paste it into a spreadsheet or other document");
						return;
				  } );
					clip.addEventListener( 'onLoad', function(){
															  var thzclp=clip;
															  alert('zeroClipboard loaded'+node);
															  } );
					clip.glue(nd);
												 },5000);
				  };
				  makeClipButton(""+node+"_copy","boo"+node);
...
//alert(_dso[dataSrc].fields.length+":"+_dso[dataSrc].fields[0].sTitle);
			  _dso[dname].data=Fn(oba.length>0? _dso[dataSrc].data.sort(sortFn):_dso[dataSrc].data);
			  return _dso[dname];
		  },
