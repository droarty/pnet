...
var processComposites = function() {
  ...
  schoolsQuery=`
    ...
    on sch.cdts = z.cdts
    left join (
      select year, cdts, count(*) totComps from schwideComposites where subj='Reading' group by year, cdts
  ...
  connection.query(schoolsQuery, function(err, rows, fields){
    ...
    async.eachSeries(rows, function(row, callback) {
      ...
      var year = row['year']
      console.log(`${cnt++}) Start ${cdts}, ${year} at ${currTime()}`)
...
//  queries += " union " + query_for_parcc;
  ...
  var q = `
  insert IGNORE into schwideComposites
  ...
  from (
    select h.*, q3pct+q4pct paaavg,
     ...
     from (
      ${queries}
     where cohort is not null /* this eliminates all hs scores */
    ) h
