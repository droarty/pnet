// NOTE:  This takes about 15 hours to run if rebuilding whole table
// by joining with isbe_tests we have subject = math, science and reading.
// we then get a math composite and a reading composite
// then we get a composite across subjects, but only for 3rd to 8th
// This script then builds the schoolComposites table.
...
var processComposites = function() {
  ...
  schoolsQuery=`
    select z.year, sch.shortlabel, tcnt, z.cdts zcdts, totComps
    from  (
      select year, cdts, count(*) tcnt from isbedetails group by year, cdts
      union
      select year, cdts, count(*) tcnt from parccdetails group by year, cdts
    ) z
    left join tblsitemaster sch
    on sch.cdts = z.cdts
    left join (
      select year, cdts, count(*) totComps from schwideComposites group by year, cdts
  ) cmp on (cmp.year = z.year and cmp.cdts = z.cdts)
    where (z.cdts is not null and tcnt > 1 and cmp.totComps is null)
  ...
  connection.query(schoolsQuery, function(err, rows, fields){
    ...
    async.eachSeries(rows, function(row, callback) {
      ...
      var year = row['year']
      var searchStr = "cdts='" + cdts + "' and year="+year;
      var schName = (row['shortlabel'] || "Missing Name").replace("'", "''")
      ...
      connection.query(q, function(err, rows, fields) {
        if (err) console.log(`SchQuery Error: ${err} ->  q= ${searchStr}`)
  ...
  var query_for_isat_and_psae = `
    select g.*,'8=8' q, '|test|' test,
      ...
      totalNotBelow13 totalWarning13, totalNotMeeting13-totalNotBelow13 totalBelow13,totalNotExceeding13-totalNotMeeting13 totalMeet13, if(totalNotExceeding13>0,s9totToHere-totalNotExceeding13,0) totalExceed13,
       ...
       if(s9totToHere > 0, round((s1totToHere-s1deduct)/s9totToHere*100,1), 0) s1pct,
    ...
    from (
    select d.year, d.gradeLevel,if(s.gradeLevel in ('03', '04', '05', '06', '07', '08'), d.year-s.gradeLevel+12, null) cohort,
    ...
    where totalToHere > 0 and d.gradeLevel in ('03', '04', '05', '06', '07', '08')
  ...
  query_for_parcc = `
    select g.*,'8=8' q, 'PARCC' test,
      ...
      0 totalWarning13, 0 totalBelow13, 0 totalMeet13, 0 totalExceed13,
       ...
       round(p5totToHere-p4totToHere+p4deduct,1) p5cnt
    from (
    select d.year, s.gradeLevel,if(s.gradeLevel in ('03', '04', '05', '06', '07', '08'), d.year-s.gradeLevel+12, null) cohort,
      ...
      where totalToHere > 0 and s.gradeLevel in ('03', '04', '05', '06', '07', '08')
      group by d.year, s.gradeLevel, d.ptest
    ) g
  `;

  queries += " union " + query_for_parcc;
//  queries =  query_for_parcc;

  var q = `
  insert into schwideComposites
  (subj, rcdts, shortlabel, cdts, year, maxMaxScore, minMinScore, wAvgScore, totalCount, wAvgPercentileEquivalent,wAvgoldPctEquiv,wAvgZScore, stanine, PME, paa60, paa68, s1pct, s2pct, s3pct, s4pct, s5pct, s6pct, s7pct, s8pct, s9pct, q1pct, q2pct, q3pct, q4pct, p1pct, p2pct, p3pct, p4pct, p5pct, totalWarning, totalBelow, totalMeet, totalExceed, cntaa60, cntaa68, s1cnt, s2cnt, s3cnt, s4cnt, s5cnt, s6cnt, s7cnt, s8cnt, s9cnt, q1cnt, q2cnt, q3cnt, q4cnt, p1cnt, p2cnt, p3cnt, p4cnt, p5cnt)
  ...
  round(sum(h1.cntaa60)/sum(h1.totalCount)*100,1) paa60, round(sum(h1.cntaa68)/sum(h1.totalCount)*100,1) paa68,   round(sum(h1.s1cnt)/sum(h1.totalCount)*100,1) s1pct, round(sum(h1.s2cnt)/sum(h1.totalCount)*100,1) s2pct, round(sum(h1.s3cnt)/sum(h1.totalCount)*100,1) s3pct, round(sum(h1.s4cnt)/sum(h1.totalCount)*100,1) s4pct, round(sum(h1.s5cnt)/sum(h1.totalCount)*100,1) s5pct, round(sum(h1.s6cnt)/sum(h1.totalCount)*100,1) s6pct, round(sum(h1.s7cnt)/sum(h1.totalCount)*100,1) s7pct, round(sum(h1.s8cnt)/sum(h1.totalCount)*100,1) s8pct, round(sum(h1.s9cnt)/sum(h1.totalCount)*100,1) s9pct,
  ...
  sum(h1.cntaa60) cntaa60, sum(h1.cntaa68) cntaa68,
